  ...
  if (!message) {
    return (
      <div className="flex-1 flex items-center justify-center p-8">
        <p className="text-sm text-[#6E7075]">Message not found</p>
      </div>
    )
  }
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-[#26282C]">
        <div className="flex items-center gap-3 mb-3">
          <button onClick={() => router.back()} className="p-1.5 rounded-[8px] hover:bg-[#1E2024] text-[#6E7075] md:hidden">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-sm font-semibold text-white truncate">{message.sender}</h2>
            <p className="text-xs text-[#6E7075]">{message.senderEmail} · {formatDate(message.timestamp)}</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {message.htmlBody ? (
          ...
          <div className="text-sm text-[#AEB0B4] leading-relaxed whitespace-pre-wrap">
      ...
      {/* Actions */}
      <div className="px-6 py-4 border-t border-[#26282C] flex items-center gap-3">

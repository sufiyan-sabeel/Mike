  ...
  if (messages.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
        ...
        <p className="text-sm text-[#AEB0B4] font-medium">Waiting for your first message</p>
        <p className="text-xs text-[#6E7075] mt-1">Send an email to your temporary address</p>
      </div>
    )
  }
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={cn('text-sm font-medium truncate', msg.read ? 'text-[#AEB0B4]' : 'text-white')}>{msg.sender}</span>
              </div>
              <p className="text-sm text-[#AEB0B4] truncate">{msg.subject}</p>
              <p className="text-xs text-[#6E7075] truncate mt-0.5">{msg.preview}</p>
            </div>
